...
class TopologyGui(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(cons, text="最大失效节点数").pack(side=tk.LEFT)
        ...
        ttk.Label(right, text="结果").pack(anchor="w")
        self.result_text = tk.Text(right, height=20, width=36)
    ...
    def generate_grid(self):
        try:
            ...
            messagebox.showerror("错误", "行列请输入整数")
            return
        if rows <= 0 or cols <= 0:
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                self.nodes.append(Node(id=nid, reliability=float(self.node_rel.get())))
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if c + 1 < cols:
                    self._add_edge(cur, self.nodes[index(r, c + 1)].id)
                if r + 1 < rows:
                    self._add_edge(cur, self.nodes[index(r + 1, c)].id)
    ...
    def _add_edge(self, s, t):
        ...
        self.edge_id_seed += 1
        self.edges.append(Edge(id=eid, source=s, target=t, reliability=float(self.edge_rel.get())))
    ...
    def on_canvas_click(self, event):
        ...
        if self.pending_edge_node is None:
            ...
            if hit != self.pending_edge_node:
                self._add_edge(self.pending_edge_node, hit)
                self.pending_edge_node = None
                self.redraw()
    ...
    def redraw(self):
        ...
        for n in self.nodes:
            ...
            self.canvas.create_oval(x - 12, y - 12, x + 12, y + 12, fill="#5aa9ff", outline="#1e4b7a")
            self.canvas.create_text(x, y - 18, text=n.id, font=("Arial", 9))
    ...
    def compute(self):
        try:
            ...
            topo = Topology(
                nodes=self.nodes,
                edges=self.edges,
                constraints=Constraints(max_fail_nodes=max_fail, nodes_max_hops=hops),
            )
